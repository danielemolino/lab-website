#!/usr/bin/env python3
"""Push the local education CSV into the shared Google Sheet."""
...
DEFAULT_SYNC_CONFIG = ROOT / "shared" / "education_sync.json"
...
def extract_sheet_identifiers(source_url: str) -> tuple[str, int]:
    ...
    if not match:
        raise ValueError("Could not extract spreadsheet id from education_sync.json source URL.")
...
def main() -> int:
    ...
    if not source_url:
        return fail("shared/education_sync.json does not contain a `source` URL.")
    ...
    print(
        f"Pushed {max(len(values) - 1, 0)} education rows from {source} "
