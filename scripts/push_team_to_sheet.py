#!/usr/bin/env python3
"""Push the local team CSV into the shared Google Sheet.

This is intentionally isolated from the rest of the site. It only:
- reads `shared/team_sheet.csv`
- resolves spreadsheet id + gid from `shared/team_sync.json`
...
def extract_sheet_identifiers(source_url: str) -> tuple[str, int]:
    ...
    if not match:
        raise ValueError("Could not extract spreadsheet id from team_sync.json source URL.")
...
def main() -> int:
    ...
    if not source_url:
        return fail("shared/team_sync.json does not contain a `source` URL.")
